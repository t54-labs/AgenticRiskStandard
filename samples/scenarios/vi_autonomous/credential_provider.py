#!/usr/bin/env python3
"""VI autonomous mode: Credential Provider — issues L1, verifies chains."""
...
def cmd_l1(state: dict) -> None:
    ...
    resp = client.issue_l1(
        state["job_id"], state["agreement_hash"], l1_serialized="l1-identity"
    )
...
def cmd_verify_chain(state: dict) -> None:
    sk = SigningKey(bytes.fromhex(state["cred_provider_key"]))
    client = VICredentialProviderClient(base_url=SERVER, signing_key=sk)
    resp = client.verify_chain(state["job_id"], state["agreement_hash"])
...
if __name__ == "__main__":
    ...
    if cmd == "keys":
        ...
        {"l1": cmd_l1, "verify-l2": cmd_verify_l2, "verify-chain": cmd_verify_chain}[
            cmd
        ](state)
